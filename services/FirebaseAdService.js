...
/**
 * 앱 시작(스플래시) 시점에 호출하는 광고 '데이터' 워밍업.
 * - 광고 설정(어떤 광고를 어디에 띄울지)을 디스크/메모리 캐시로 미리 끌어올려
 *   첫 화면이 그려질 때 광고 목록이 이미 준비되게 함.
 * - getRawDocs가 "캐시가 1시간 이상 묵었으면 백그라운드 갱신"까지 알아서 처리.
 * - 이미지 자체의 캐싱/프리로드는 expo-image(네이티브 SDWebImage)가 담당하므로
 *   여기서 이미지를 prefetch하지 않는다. (RN Image.prefetch는 iOS에서 버그 — 제거)
 * fire-and-forget — 절대 throw 안 함.
 */
export const prefetchAppAds = async () => {
  try {
    await getRawDocs(); // 광고 설정 디스크/메모리 워밍 + 필요 시 백그라운드 갱신
  } catch (_) {}
};
