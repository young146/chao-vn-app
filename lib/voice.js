...
let AvAudio = null;   // 오디오 세션 제어 (expo-av — 모든 기존 바이너리에 이미 들어 있음)
...
try { AvAudio = require('expo-av').Audio; } catch (_) { /* 없어도 동작엔 지장 없음 */ }

// ── iOS 오디오 세션 되돌리기 ─────────────────────────────────────────
// 🚨 iOS 는 "지금 소리를 어떻게 다룰 것인가"(오디오 세션)를 앱이 하나 갖고 있는데,
//    **음성인식이 이걸 녹음 모드로 바꿔 놓는다**(라이브러리가 AVAudioSession 을 직접 조작한다).
//    녹음 모드인 채로 TTS 를 틀면 — 소리가 안 나거나, 작거나, 수화기(귀에 대는 스피커)로 나간다.
//    안드로이드에서 겪은 "무음" 사고의 iOS 판이 이 자리다.
// 또 하나: iOS 는 **무음 스위치**(옆면 스위치)가 켜져 있으면 기본 설정으로는 TTS 도 침묵한다.
//    어르신들은 무음으로 두는 경우가 많다 → playsInSilentModeIOS 로 무음 모드에서도 소리 나게 한다.
// 그래서: 인식이 끝날 때마다 "세션이 더러워졌다"고 표시해 두고,
//    다음 speak 직전에 **재생 모드로 되돌린 뒤** 읽는다. (한 번 되돌리면 다시 더러워질 때까지 건너뜀)
let iosAudioDirty = true;   // 처음에도 한 번은 맞추고 시작한다
async function ensurePlaybackIOS() {
  if (Platform.OS !== 'ios' || !iosAudioDirty || !AvAudio) return;
  try {
    await AvAudio.setAudioModeAsync({
      playsInSilentModeIOS: true,    // 무음 스위치가 켜져 있어도 읽어준다
      allowsRecordingIOS: false,     // 녹음 모드 해제 → 스피커로 정상 재생
    });
    iosAudioDirty = false;
  } catch (_) { /* 실패해도 speak 는 시도한다 — 안 되면 probeSpeech 가 이유를 밝힌다 */ }
}
...
export function startListening({ onPartial, onFinal, onError, onEnd, lang = 'ko-KR' } = {}) {
  ...
  try {
    ...
    subs.push(SR.addListener('error', (e) => {
      iosAudioDirty = true;          // 인식이 세션을 만졌다 — 다음 읽기 전에 되돌린다
      if (done) return;
      done = true;
      off();
...
export async function speak(text, { onDone, onError, lang = 'ko-KR', rate = 0.95, queue = false } = {}) {
  ...
  try {
    await ensurePlaybackIOS();       // iOS: 인식이 바꿔 놓은 세션을 재생 모드로 되돌린다
    if (!queue) {
...
export async function probeSpeech() {
  ...
  try {
    ...
    // 설정 경로가 두 플랫폼이 다르다 — 엉뚱한 길을 알려주면 더 헤맨다
    const path = Platform.OS === 'ios'
      ? '설정 → 손쉬운 사용 → 콘텐츠 말하기 → 음성'
      : '설정 → 접근성 → 텍스트 음성 변환';
    if (list.length === 0) {
      return { ok: false, total: 0, ko: 0, reason: `이 폰에 음성(TTS) 엔진이 없습니다. ${path} 에서 설치해 주세요.` };
    }
    if (ko.length === 0) {
      return { ok: false, total: list.length, ko: 0,
        reason: `한국어 음성이 설치돼 있지 않습니다(설치된 음성 ${list.length}개). ${path} 에서 한국어를 받아 주세요.` };
    }
...
export function createSpeechStream({ onStart, onIdle, onError, rate = 0.95, lang = 'ko-KR' } = {}) {
  ...
  return {
     ...
     *  처음부터 다시 세되, **이제부터 오는 글은 확정된 답**이므로 읽기를 연다.
     *  ⚠️ stop 은 **실제로 읽던 게 있을 때만** 부른다 — 안 읽는데 stop 을 던지는 습관이
     *     안드로이드 무음 사고(stop 이 speak 를 죽임)의 뿌리였다. 필요 없는 stop 은 아예 안 한다. */
    reset() {
      const wasSpeaking = pending > 0 || spokenLen > 0;
      spokenLen = 0; pending = 0; armed = true;
