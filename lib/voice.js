...
export function speak(text, { onDone, lang = 'ko-KR', rate = 0.95, queue = false } = {}) {
  if (!isSpeakSupported()) { if (onDone) onDone(); return; }
  const t = speakableText(text);
  if (!t) { if (onDone) onDone(); return; }
  try {
    if (!queue) Speech.stop();   // 앞엣것이 읽히는 중이면 끊고 새로 읽는다
    Speech.speak(t, {
      language: lang,
 ...
 *   sp.stop();                   // 화면을 떠날 때
 */
export function createSpeechStream({ onIdle, rate = 0.95, lang = 'ko-KR' } = {}) {
  ...
  return {
    ...
    /** 앞서 읽던 것은 버려진 서두였다 — 멈추고 처음부터 다시 센다. */
    reset() { spokenLen = 0; pending = 0; stopSpeaking(); },
