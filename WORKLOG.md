...
## 2026-09-04 (오후) — 🔍 [블로그] 색인 0의 진짜 원인 규명 · 본진에서 블로그로 길 내기

> 저장소: `daily-news-final` · `vnkorlife-web`

### 사장님 지적으로 잘못된 진단을 두 번 바로잡았다

처음에 **"새 도메인이라 느리다 · 본진 링크가 없어서"** 라고 답했다. **짐작이었다.**
사장님: *"처음 3개는 바로 색인됐는데 왜 나머지는 그대로인가? 제대로 조사해봐라."*
→ `scripts/gsc-inspect.js` 로 **16편 전수 조회**하니 전혀 다른 그림이 나왔다.

| 상태 | 편수 |
|---|---|
| 🔴 리디렉션 오류 | 10 |
| 발견됨 - 크롤 안 함 | 3 |
| 구글이 아직 모르는 URL | 3 (9월 글) |
| ✅ 색인 | **홈 1쪽뿐** |

**두 번째 짐작도 틀렸다.** `?m=1` 모바일 리디렉션이 원인이라 몰아갈 뻔했는데
(인터넷 커뮤니티가 전부 그렇게 말한다), **홈페이지도 똑같이 `?m=1` 로 302 를 주면서
색인되어 있다.** 같은 사이트 안에 반증이 있었다. 검색 결과를 그대로 믿었으면
엉뚱한 걸 고칠 뻔했다.

### 측정으로 밝힌 것
- **9월 글 3편이 "미발견" 인 이유**: 구글 크롤 9/3 02:03~02:12 UTC → 글 발행 9/3 08:33~08:37 UTC.
  **구글이 다녀간 뒤에 올린 글이다.** 오류가 아니라 순서 문제.
- HTTPS 인증서 발급 9/3 00:11 UTC → 크롤이 그 2시간 뒤. 전환 직후에 훑고 갔다.
- 서버는 `Googlebot` 과 `Google-InspectionTool` 을 **응답 헤더까지 똑같이** 대한다(측정 완료).
- 라이브 테스트는 통과(사장님 스크린샷). canonical 정확, 루프 없음, robots 허용.

### 🔴 아직 설명 못 하는 것
**9/4 06:31 자연 재크롤도 실패로 기록**됐는데 25분 뒤 라이브 테스트는 성공했다.
차이는 구글 내부에 있고 밖에서는 볼 수 없다. → **3~4일 지켜보고 계속 실패하면 다시 판다.**

### 조치 — 본진에서 블로그로 가는 길을 냈다 (링크 0개였다)
- **chaovietnam.co.kr**: 기사 맨 아래 자매 사이트 한 줄 (`xinchao_cta_sisters`). 25,000개 기사 전부.
  ⚠️ **배너(house 소재)로 만들지 않았다** — house 소재는 `XINCHAO_HOUSE_MAX`(2칸)를 놓고
  **유료 광고와 경쟁한다.** 구글은 배너든 글자든 `<a href>` 를 똑같이 따라가므로
  목적은 같고 광고 매출을 안 깎는 쪽을 골랐다.
- **vnkorlife.com**: 사이트 전체 푸터 + `/xinchao` 창구 칸. 번역 3개 언어(ko·en·vi) 전부.
- ⚠️ **`nofollow` 를 넣지 말 것.** 넣으면 링크를 건 이유 자체가 사라진다.

**실서버 확인 완료**: WP 기사에 한 줄 노출 · vnkorlife 5개 페이지 전부 노출.

📌 **링크는 "찾아올 길" 이지 "색인해달라" 는 명령이 아니다.** 색인이 빨라진다고
   장담할 수 없다. 다만 길이 한 줄도 없던 것보다는 낫다.

### 다음 단계
- **9월 글 3편만** 서치콘솔에서 색인 요청 (나머지 13편은 구글이 스스로 재방문 중이라 할당량을 아낀다)
- 3~4일 뒤 `node scripts/gsc-inspect.js --site https://www.vietnamsari.com/ <URL들>` 로 재확인
- 계속 실패하면 Googlebot 에게만 나타나는 실제 문제 → 재조사
