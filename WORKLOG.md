...
## 2026-08-08 — 🟠 [SEO] 전면 감사 + 구글 뉴스 사이트맵·편집부 표기 신설 (FTP 업로드 대기)

- **한 일**: 사장님 요청으로 구글·네이버 SEO 를 실물 조회로 전면 감사했다. **기술 SEO 는 이미 합격점**
  (색인률 99.4% = 26,600/26,750, Rank Math 정상, 네이버 인증·색인 정상, 기사 TTFB 0.29초).
  대신 **콘텐츠 구성이 최대 리스크**임을 확인: 전체 26,797건 중 데일리뉴스 19,707건(73.5%),
  최근 30일은 97.2%(하루 약 47건). 구글이 2024-03 스팸정책에 신설한 **'대규모 콘텐츠 남용'** 은
  *자동 번역 페이지*를 위반 예시로 명시하고 있고 2026-03 코어 업데이트가 이를 정조준했다.
  → 수동 조치는 **"감지된 문제 없음"** 확인(사장님 화면). 지금은 제재 전 = **예방 단계**.
...
- **다음 단계**:
  ...
  4. 남은 미조치: 홈·카테고리가 LiteSpeed 캐시 제외라 2.0~2.6초 (기사는 0.29초). ESI 방식 검토
