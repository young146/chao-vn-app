...
- **배포**: 플러그인 FTP(수차례) + 앱 OTA **group `d4693711`** (production rv 2.4.3). 실서버 전수 검증 완료(10개 뭉치 대조 일치, 슬러그 전부 `issue-NNN`).
- **상태**: 🟢 배포완료 / ⏳ 8월 초 565호 기사 업로드 시 실물 확인
- **다음 단계**: **Phase 3 = EAS Build**(iOS Analytics·이미지 최적화·측정결함 4-F·푸시 이미지). 사장님 방침: iOS/Android 동시 버전업.
