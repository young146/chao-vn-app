...
- **배포**: 앱 OTA `b65730a8` (production, rv 2.4.3) · 커밋 `743eebd`
- **상태**: 🟡 **앱은 배포완료, 서버 규칙 배포 대기** — `firestore.rules` 에 `reports` 규칙을 추가했으나 **사장님이 `firebase deploy --only firestore:rules` 를 실행해야** 신고가 저장된다. 그 전에는 차단만 동작(신고는 화면상 접수되나 저장 실패).
- **다음 단계**: ① 규칙 배포 → ② 신고 1건 테스트 → ③ 신고 내역 확인 화면(관리자)은 아직 없음. Firebase 콘솔에서 `reports` 컬렉션을 직접 보거나, 필요하면 회원관리 화면에 붙인다.
- **관련 파일**: [services/moderationService.js](services/moderationService.js) · [firestore.rules](firestore.rules)
