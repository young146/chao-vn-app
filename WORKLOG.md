...
- **안 고친 것**: 맨 위 카드에 8/10 같은 옛 날짜가 보이는 건 **서버 backfill 의 정상 동작**이다
  (오늘치가 0건인 섹션을 과거 기사로 채운다 — 오늘 기준 `politics`·`korea_vietnam` 이 그렇다).
  대표카드에도 "지난 기사" 표시를 붙일지는 **사장님 판단 대기**.
- **배포**: 앱 OTA (production) — 순수 JS, 네이티브 변경 없음. runtimeVersion 2.4.3 (마지막 빌드와 일치)
- **상태**: ⏳ 안드로이드 실기기 확인 대기
- **다음 단계**: 안드로이드에서 ① 첫 진입 ② 날짜 바꾸기 ③ 당겨서 새로고침 3가지 확인. 여전히 비면 화면에 뜨는
  문구가 "뉴스를 불러오지 못했습니다"(= 통신/저장 실패) 인지 "등록된 뉴스가 없습니다"(= 그날 지면 없음) 인지 알려줄 것 — 이제 구분된다.
