...
### ⚠️ 아직 결정이 필요한 것 — 사이드바의 신풍제약 · CIS

살아있는 e3lan 은 **사이드바 위젯 2개뿐**이고, 거기 **신풍제약**과 **CIS 국제학교** 광고가 돌고 있다.
`e3lan` 은 광고차단 필터에 등재된 클래스다(§5-1 실측) → **광고차단 쓰는 독자(한국 기준 20~30%)에게 이 둘은 안 보인다.**
노출수를 광고주에게 실제보다 부풀려 보고하고 있는 셈이고, 그 둘의 클릭 성과는 아예 집계되지 않는다.

**'옮긴다'는 위치 이동이 아니다.** 두 광고는 이미 사이드바에 있다. **감싸는 껍데기만 바꾸는 것**이다.

```
지금  : 사이드바 ▸ [테마 위젯 .e3lan] ▸ 신풍제약 이미지     ← .e3lan 이라 차단당한다
바꾸면: 사이드바 ▸ [우리 슬롯 .xc-slot] ▸ 신풍제약 이미지   ← 같은 자리·같은 폭·같은 이미지
```

**이미지 크기는 다시 만들 필요가 없다.** 실제 파일은 신풍제약 **1063×756**, CIS **1600×2221** 로
통합센터 권장(600×500)과 안 맞지만, 우리 슬롯은 `width:100%; height:auto` 라
**원본 비율 그대로 사이드바 폭에 맞춰 줄어들 뿐 자르지 않는다.** 지금 테마 위젯이 하는 것과 같다.

| 얻는 것 | 드는 품 |
|---|---|
| 광고차단 회피 · GA4 노출/클릭 집계 · 기간·우선순위 관리 | 테마 위젯에서 빼고 통합센터에 이미지·링크·기간 한 번 등록 |

**비어 있는 e3lan 5곳**(`top`·`below_header`·`home-e3lan`·`post`×2·`bottom`)은 잃을 것이 없으니
테마 옵션에서 꺼도 된다. 다만 테마 설정이라 코드로는 못 끈다.
