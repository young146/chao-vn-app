...
| 🎨 `e3lan` 사이드바 위젯 ×2 | 홈·상세 | **신풍제약 · CIS 국제학교** (실제 광고) | 🟢 살아있음 — 건드리지 말 것 |
| 🎨 `e3lan-top` | 홈·상세 | 애드센스 | 🟢 살아있음 |
| 🎨 `e3lan-below_header` | 홈·상세 | 이미지 배너 | 🟡 통합 `top` 과 근접 |
| 🎨 `e3lan-post` | 상세 | 이미지 | 🟡 |
| 🎨 `e3lan-bottom` | 홈·상세 | **비어있음** | 🟡 테마 옵션이라 코드로 못 끈다 |
...
### ⚠️ 아직 결정이 필요한 것 — 테마 `e3lan` 칸

**신풍제약·CIS 국제학교 광고가 `e3lan` 클래스 안에서 돌고 있다.**
`e3lan` 은 광고차단 필터에 등재된 클래스다(§5-1) → **광고차단 쓰는 독자(한국 기준 20~30%)에게 이 두 광고는 안 보인다.**
노출수를 광고주에게 실제보다 부풀려 보고하고 있는 셈이다.

→ 통합 광고센터의 `sidebar` 슬롯으로 옮기면 해결되지만, **지금 도는 광고를 손대는 일**이라 사장님 결정이 필요하다.
   (`e3lan-bottom` 은 비어 있으므로 테마 옵션에서 꺼도 잃을 것이 없다.)
