 ...
 * Description: Rank Math 가 채우지 못하는 두 구멍을 메운다 — (1) 구글 뉴스 전용 사이트맵, (2) 데일리 뉴스의 "편집부 번역·정리" 표기, (3) 구조화 데이터에 원문 출처 신고, (4) 탐색경로가 하위 카테고리까지 내려가게, (5) headline 에서 사이트명 제거, (6) 뉴스 섹션 페이지(/news/economy/ 등) 신설, (7) 크롤링 예산 정리(글별 피드 링크 제거·홈 가짜 페이지번호 301).
...
// ============================================================
// 7) 크롤링 예산 정리  —  구글이 헛걸음하는 주소를 없앤다
// ------------------------------------------------------------
// 왜 필요한가 (2026-09-03 서치콘솔 실측):
//   미색인 4.95만 건 중 두 덩어리가 "구글이 읽으러 왔지만 넣을 게 없던" 주소였다.
//
//   (a) 글별 댓글 피드 15,547건 — 「크롤링됨 - 현재 색인이 생성되지 않음」의 대부분.
//       워드프레스는 글 하나마다 전용 댓글 RSS(/{글번호}/feed/)를 만들고 그 링크를
//       <head> 에 박는다. 댓글이 0건이어도 박힌다. 기사 27,918편 = 피드 2.8만 개.
//       피드는 애초에 검색에 나오는 물건이 아니라 색인 안 되는 게 정상이지만,
//       구글은 그걸 확인하려고 2.8만 번 기어들어온다. 그 시간에 새 기사를 읽어야 한다.
//       (이 사이트는 데일리 뉴스를 하루 약 47건 발행한다 — 크롤 시간이 아깝다.)
//
//   (b) 홈 목록의 가짜 페이지 번호 — /page/2/ 부터 /page/99999/ 까지 전부 200 을 준다.
//       실측: 홈·page/2·page/3·page/99999 가 **똑같은 글 65개**를 보여준다.
//       즉 홈 페이지네이션은 작동하지 않는데 주소만 무한히 열리는 상태다.
//       구글 입장에선 기어다닐 주소가 끝이 없다.
//
// 무엇을 하는가 / 무엇을 안 하는가:
//   · 글별·분류별 피드는 <head> 링크만 뺀다. 피드 주소 자체는 그대로 살아 있다.
//   · 사이트 전체 피드(/feed/, /comments/feed/)는 **건드리지 않는다.**
//     뉴스 리더·외부 서비스가 쓸 수 있는 정식 창구다.
//   · robots.txt 로 막지 않는다. 막으면 구글이 "차단됨"으로 재분류만 할 뿐이고,
//     사이트 전체 피드까지 휩쓸릴 위험이 있다. 링크를 끊는 쪽이 안전하다.
//
// 효과가 나타나는 속도:
//   이미 구글이 아는 2.8만 개가 즉시 사라지지는 않는다. 재크롤하면서 몇 주에 걸쳐 준다.
//   서치콘솔 「크롤링됨 - 색인 생성되지 않음」 숫자가 내려가는 것으로 확인한다.
// ============================================================

// (a) 글별·분류별 피드 링크를 <head> 에서 뺀다.
//     feed_links_extra = 글/카테고리/태그/작성자/검색 별 피드 링크 (우선순위 3 으로 등록돼 있다)
//     feed_links       = 사이트 전체 피드 링크  ← 이건 남긴다
remove_action('wp_head', 'feed_links_extra', 3);

// (b) 홈 목록의 페이지 번호 주소를 홈으로 되돌린다.
//     ⚠️ 이 처리는 "홈 페이지네이션이 작동하지 않는다"는 실측에 근거한다.
//        나중에 홈 목록에 진짜 페이지 넘김을 붙이게 되면 이 블록을 먼저 지워야 한다.
add_action('template_redirect', 'chaovn_seo_kill_fake_home_paging');
function chaovn_seo_kill_fake_home_paging() {
