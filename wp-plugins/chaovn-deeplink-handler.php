...
// ============================================================
// 1. .well-known 파일 서빙 (App Links / Universal Links 인증)
//    init 훅에서 WordPress보다 먼저 가로채서 JSON 응답
// ============================================================
add_action('init', function () {
    $uri = isset($_SERVER['REQUEST_URI']) ? strtok($_SERVER['REQUEST_URI'], '?') : '';

    // Android App Links 인증 파일
    if ($uri === '/.well-known/assetlinks.json') {
        ...
        echo json_encode([[
            ...
            'target'   => [
                ...
                'package_name'            => CHAOVN_ANDROID_PACKAGE,
                'sha256_cert_fingerprints' => [CHAOVN_ANDROID_SHA256],
            ],
        ]], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
        exit;
    }

    // iOS Universal Links 인증 파일
    if ($uri === '/.well-known/apple-app-site-association') {
        ...
        echo json_encode([
            'applinks' => [
                'apps'    => [],
                'details' => [[
                    'appID' => CHAOVN_IOS_TEAM_ID . '.' . CHAOVN_IOS_BUNDLE_ID,
                    'paths' => ['*'],
                ]],
            ],
        ], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
...
}, 1); // priority 1 = WordPress 라우팅보다 먼저 실행

// ============================================================
// 2. Rewrite Rules 등록 (danggn / job / realestate 공유 페이지)
// ============================================================
add_action('init', function () {
    ...
    (function () {
        ...
        function tryOpenApp() {
            if (isAndroid) {
                ...
                window.location.href = 'intent://<?php echo esc_js(CHAOVN_NEWS_TERMINAL_SLUG); ?>#Intent;scheme=chaovietnam;package=<?php echo esc_js(CHAOVN_ANDROID_PACKAGE); ?>;S.browser_fallback_url=;end';
            } else {
...
// ============================================================
// 7. 공유 페이지 렌더링 (danggn / job / realestate)
// ============================================================
        ...
        (function(){
            ...
            function openApp() {
                if (isAndroid) {
                    ...
                    intent += '#Intent;scheme=chaovietnam;package=<?php echo esc_js($android_pkg); ?>;end';
...
// ============================================================
// 8. 플러그인 활성화 / 비활성화 시 Rewrite Rules 갱신
// ============================================================
